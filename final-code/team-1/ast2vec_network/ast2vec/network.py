...
from parameters import BATCH_SIZE, NUM_FEATURES, HIDDEN_NODES

def init_net(
        NODE_MAP,batch_size=BATCH_SIZE, num_feats=NUM_FEATURES, hidden_size=HIDDEN_NODES
):
    ...
    with tf.name_scope('network'):
        ...
        with tf.name_scope('inputs'):
            ...
            embeddings = tf.Variable(
                tf.random_uniform([len(NODE_MAP)+1, num_feats]), name='embeddings'
            )
